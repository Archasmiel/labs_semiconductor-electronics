import matplotlib.pyplot as plt
import laboratory_works.library.plotio as peg
import laboratory_works.library.xlsxio as io

index1 = 0
index2 = 1

columns = [[chr(i) for i in range(ord('A'), ord('F')+1)]] * 4
rows = [[2, 22], [2, 22], [2, 30], [2, 22]]

# all_sheets = io.get_all_sheets_from_file('../../data/lab5/dataBron.xlsx', columns, rows)
all_sheets = io.get_all_sheets_from_file('../!data/data.xlsx', columns, rows)
peg.get_graph_with_errors(plt, all_sheets[index1][2], all_sheets[index1][4], all_sheets[index1][3],
                          all_sheets[index1][5], 2, 0.5, 2, 'k', True, f'Ud, В', f'Id, мА')
plt.plot([-0.1, 1.5], [0, 0], color='k')
plt.plot([0, 0], [-1, 20], color='k')
plt.title('Діод 1 - прямий')
plt.grid()


peg.get_graph_with_errors(plt, all_sheets[index2][2], all_sheets[index2][4], all_sheets[index2][3],
                          all_sheets[index2][5], 2, 0.5, 2, 'k', True, f'Ud, В', f'Id, мА')
plt.plot([-0.5, 0.1], [0, 0], color='k')
plt.plot([0, 0], [-20, 1], color='k')
plt.title('Діод 1 - зворотний')
plt.grid()


plt.show()
